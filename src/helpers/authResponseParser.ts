...
type PersistedAuthContextData = Omit<ParsedAuthContext, "tokens" | "lastLoginAt">

type PersistedEnvelope = {
  version: string
  encrypted: boolean
  data: PersistedAuthContextData
  timestamp: number
  expiresAt: number
}
 ...
 */
export function parseIMEIString(imeiString: string) {
 ...
 */
export function parseAuthResponse(response: AuthResponseInput): ParsedAuthContext {
 ...
 */
async function encryptDataAsync(text: string, key: string): Promise<string> {
 ...
 */
function validateUserContext(context: ParsedAuthContext): boolean {
 ...
 */
export async function persistUserContextAsync(userContext: ParsedAuthContext): Promise<boolean> {
  try {
    // Validate input
 ...
 */
export function persistUserContext(userContext: ParsedAuthContext): boolean {
  try {
    // Validate input
    if (!validateUserContext(userContext)) {
      console.error('Invalid user context: cannot persist');
...
export function loadUserContext() {
  try {
    ...
    const persistedData = JSON.parse(decrypted) as PersistedEnvelope;
